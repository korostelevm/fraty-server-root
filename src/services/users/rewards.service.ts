...
const ClaimReward = async (wallet: string, _id: string) => {
	...
	if (CheckPoints?.balance < Reward.points) return { error: "Insufficient points" };
	if (CheckPoints?.level < Reward.level) return { error: "Not Eligible to Claim Reward" };
	const check = await UserRewards.findOne({ wallet, reward_id: _id, isRedeemed: true });
...
const UpdateReward = async (code: any) => {
	...
	if (reward.reward_type == "digital") return { error: "It is an digital Reward" };
...
const queue = Consumer.create({
	...
	handleMessage: async (message: any) => {
		...
		const balance = await contract.methods.balanceOf(wallet,nft?.tokenID).call();
		if(balance > 0) {
			await UserRewards.updateOne({ code }, { isRedeemed: true });
